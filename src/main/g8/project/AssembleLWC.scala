...
object AssembleLWC {
  ...
  val setAssemblyKey =
    assembleLWC in Compile := {
      ...
      val pluginFilePath = new File(lwcPluginsFolder, "$connectorType$.plugin").toPath
      ...
      // `plugins` folder.
      val resolution =
        Resolution(quasar.$connectorType$.project.Dependencies.lwcCore.map(moduleIdToDependency).toSet)
